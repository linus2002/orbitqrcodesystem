...
test('a first check of a released code is genuine and shows the leaflet', () => {
  const r = verification.verify(codes[0], ctx());
...
test('a second check from a different device is flagged as a duplicate', () => {
  verification.verify(codes[0], ctx('198.51.100.1'));
  const r = verification.verify(codes[0], ctx('203.0.113.9'));
...
test('re-checking from the same device inside the grace window stays genuine', () => {
  const first = verification.verify(codes[0], ctx('198.51.100.1'));
  const second = verification.verify(codes[0], ctx('198.51.100.1'));
  const third = verification.verify(codes[0], ctx('198.51.100.1'));
  ...
  const row = db.get('SELECT scan_count, verified_count FROM codes WHERE code = ?', [codes[0]]);
  assert.equal(row.verified_count, 1);
  // ...and must not create an alert.
  assert.equal(db.scalar('SELECT COUNT(*) FROM alerts'), 0);
});

test('a failed scan does not make the first genuine scan look like a duplicate', () => {
  ...
  freshDb();
  db.run(`INSERT INTO products (sku, name, manufacturer) VALUES ('AMX25','Amoxicillin','N')`);
  db.run(
  ...
  const code = db.get('SELECT code FROM codes WHERE unit_index = 0').code;

  const early = verification.verify(code, ctx());
  ...
  const real = verification.verify(code, ctx());
  assert.equal(real.result, 'genuine', 'the first real patient scan must be genuine');
  assert.equal(real.scanNumber, 1);
});

test('an unknown code is flagged and raises an alert', () => {
  const r = verification.verify('AMX25-260921-00483-K7', ctx());
  ...
  const r2 = verification.verify(fabricated, ctx());
  assert.equal(r2.result, 'flagged');
  assert.equal(r2.reason, 'unknown_code');
  assert.equal(db.scalar(`SELECT COUNT(*) FROM alerts WHERE type = 'unknown_code'`), 1);
});

test('a mistyped code is invalid, not flagged as counterfeit', () => {
  const good = codes[0];
  const typo = `${good.slice(0, -1)}${good.at(-1) === '0' ? '1' : '0'}`;
  const r = verification.verify(typo, ctx());

  assert.equal(r.result, 'invalid', 'a typo must never be presented as a counterfeit');
  assert.equal(r.reason, 'checksum_failed');
  assert.match(r.message, /mistyped/i);
  assert.equal(db.scalar(`SELECT COUNT(*) FROM alerts WHERE type = 'duplicate_scan'`), 0);
});

test('a recalled batch warns the patient and raises a critical alert', () => {
  serialization.transition(1, 'distributed', {});
  serialization.transition(1, 'recalled', { reason: 'Cold chain excursion' });

  const r = verification.verify(codes[1], ctx());
  assert.equal(r.result, 'flagged');
  assert.equal(r.reason, 'recalled');
  assert.equal(r.batch.recallReason, 'Cold chain excursion');

  const alert = db.get(`SELECT * FROM alerts WHERE type = 'recalled_scan'`);
  assert.equal(alert.severity, 'critical');
});

test('an expired pack is flagged even though the code is real', () => {
  freshDb();
  const seeded = seedBasics({ expiryDays: -10 });
  const r = verification.verify(seeded.codes[0], ctx());
...
test('a code from a batch that was never released is flagged', () => {
  freshDb();
  db.run(`INSERT INTO products (sku, name, manufacturer) VALUES ('AMX25','Amoxicillin','N')`);
  db.run(
    `INSERT INTO batches (batch_number, product_id, mfg_date, expiry_date, quantity)
  ...
  const r = verification.verify(code, ctx());
  assert.equal(r.reason, 'not_released');
});

test('a voided code is refused', () => {
  db.run(`UPDATE codes SET status = 'void' WHERE code = ?`, [codes[2]]);
  const r = verification.verify(codes[2], ctx());
  assert.equal(r.result, 'flagged');
  assert.equal(r.reason, 'void');
});

test('pilot batches never pollute the live alert queue', () => {
  db.run(`UPDATE batches SET is_test = 1 WHERE id = 1`);
  verification.verify(codes[3], ctx('1.1.1.1'));
  verification.verify(codes[3], ctx('2.2.2.2')); // would normally be a duplicate alert

  assert.equal(db.scalar('SELECT COUNT(*) FROM alerts'), 0, 'sandbox scans raise no alerts');
  assert.equal(db.scalar('SELECT COUNT(*) FROM scans WHERE is_test = 1'), 2, 'but are still logged');
});

test('every attempt is written to the scan log', () => {
  verification.verify(codes[0], ctx());
  verification.verify('not-a-code', ctx());
  verification.verify(fabricateValidLookingCode(), ctx());

  const rows = db.all('SELECT result, reason FROM scans ORDER BY id');
  assert.equal(rows.length, 3);
  assert.deepEqual(rows.map((r) => r.result), ['genuine', 'invalid', 'flagged']);
});

test('repeated duplicate scans fold into one alert and escalate it', () => {
  verification.verify(codes[0], ctx('1.1.1.1'));
  for (let i = 0; i < 12; i++) verification.verify(codes[0], ctx(`10.0.0.${i}`));

  const alerts = db.all(`SELECT * FROM alerts WHERE type = 'duplicate_scan'`);
...
test('bulk verification summarises a shipment', () => {
  const summary = verification.verifyBulk([codes[0], codes[1], 'rubbish'], ctx());
  assert.equal(summary.checked, 3);
  assert.equal(summary.genuine, 2);
  assert.equal(summary.invalid, 1);
});

test('sustained failed lookups raise a code-guessing alert', () => {
  for (let i = 0; i < 10; i++) {
    verification.verify(fabricateValidLookingCode(i), ctx('192.0.2.50'));
  }
  const alert = db.get(`SELECT * FROM alerts WHERE type = 'guess_attack'`);
