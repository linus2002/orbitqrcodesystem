...
export class SanityBackend {
  ...
  /**
   * Binary content is a file asset, not a document. The upload is one request
   * from this server to Sanity, so it is not bound by what a serverless
   * function accepts from a browser; the leaflet service keeps each piece
   * under 3 MB regardless. Assets are served from Sanity's CDN by URL, and
   * are readable by anyone holding that URL even on a private dataset -
   * which is fine for a leaflet, since it is what the QR on the carton opens.
   */
  async uploadFile(bytes, { filename, contentType }) {
    const asset = await this.client.assets.upload('file', bytes, { filename, contentType });
    return { assetId: asset._id, url: asset.url, size: asset.size ?? bytes.length };
  }

  async readFile(assetId) {
    const asset = await this.client.getDocument(assetId);
    if (!asset?.url) return undefined;
    const res = await fetch(asset.url);
    if (!res.ok) throw new Error(`could not read asset ${assetId}: HTTP ${res.status}`);
